...
export default function AccountPage() {
  ...
  return <main className="mx-auto max-w-4xl px-3 py-5 sm:px-6 sm:py-8">
    <header className="mb-5 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
      <div className="flex min-w-0 items-center gap-4"><UserAvatar userId={profile.id} version={profile.avatar_version} name={profile.display_name || "牌友"} size={64} /><div className="min-w-0"><Link href="/schedule" className="text-sm font-medium text-slate-500">← 返回登记表</Link><h1 className="mt-1 break-words text-2xl font-bold">个人中心</h1><p className="mt-1 break-all text-sm text-slate-600">{profile.display_name || "牌友"}{profile.role === "admin" ? " · 管理员" : ""}</p></div></div>
