...
export default function UserPage() {
  ...
  return <main className="mx-auto max-w-3xl px-3 py-5 sm:px-6 sm:py-8">
    <header className="mb-5 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6"><div className="flex min-w-0 items-center gap-4"><Link href="/" aria-label="返回登记表" className="text-sm font-semibold text-blue-800">← 返回</Link><div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-full bg-slate-100">{data ? <UserAvatar userId={data.user.id} version={data.user.avatar_version} name={data.user.display_name} size={64} /> : <Image src="/default-avatar.svg" alt="默认头像" width={64} height={64} unoptimized />}</div><div className="min-w-0"><h1 className="break-words text-xl font-bold">{data?.user.display_name || "牌友"}</h1><p className="mt-1 text-sm text-slate-500">公开资料仅显示昵称、头像与预约。</p></div></div></header>
