...
import { updateAuthPassword, updateAuthPasswordToDefault } from "@/lib/server/auth";
...
export const PATCH = api(async (request: NextRequest, context: { params: Promise<{ id: string }> }) => {
  ...
  if (input.action === "reset_password" || input.action === "reset_default_password") {
    ...
    const outcome = input.action === "reset_default_password"
      ? await updateAuthPasswordToDefault(target.data)
      : await updateAuthPassword(target.data, input.password);
    if (outcome === "unknown") {
